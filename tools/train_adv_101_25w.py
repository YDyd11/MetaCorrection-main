...
from evaluate_cityscapes_gta import evaluate
...
ITER_SIZE = 1
NUM_WORKERS = 4
DATA_DIRECTORY = '/data12T/ydaugust/data/realworld/GTA5'
DATA_LIST_PATH = '/data12T/ydaugust/code/domain_adaptation/MetaCorrection-main/datasets/gta5_list/train.txt'
IGNORE_LABEL = 255
INPUT_SIZE = '1280,720'
DATA_DIRECTORY_TARGET = '/data12T/ydaugust/data/realworld/Cityscapes'
DATA_LIST_PATH_TARGET = '/data12T/ydaugust/code/domain_adaptation/MetaCorrection-main/datasets/cityscapes_list/train.txt'
...
POWER = 0.9
RANDOM_SEED = 1234
#RESTORE_FROM = '/data12T/ydaugust/code/domain_adaptation/MetaCorrection-main/snapshots/resnet101/GTA5_150000.pth'
RESTORE_FROM = 'http://vllab.ucmerced.edu/ytsai/CVPR18/DeepLab_resnet_pretrained_init-f81d91e8.pth'
SAVE_NUM_IMAGES = 2
SAVE_PRED_EVERY = 1000
SNAPSHOT_DIR = '/data12T/ydaugust/code/domain_adaptation/MetaCorrection-main/snapshots/resnet101_city_25w_frombegin'
WEIGHT_DECAY = 0.0005
LOG_DIR = '/data12T/ydaugust/code/domain_adaptation/MetaCorrection-main/log/train_adv_city_25w_101_frombegin'
...
def main():
    ...
    # saved_state_dict = torch.load('/data12T/ydaugust/code/domain_adaptation/MetaCorrection-main/snapshots/resnet101/GTA5_150000_D1.pth')
    # model_D1.load_state_dict(saved_state_dict)
    # saved_state_dict = torch.load('/data12T/ydaugust/code/domain_adaptation/MetaCorrection-main/snapshots/resnet101/GTA5_150000_D2.pth')
...
# 把构建的卷积网络的参数优化
    ...
    for i_iter in range(args.num_steps):
        ...
        for sub_i in range(args.iter_size):
            ...
            pred1, pred2 = model(images) # 预测两个pred 是加了不同尺度的约束
            pred1 = interp(pred1)
            pred2 = interp(pred2)

            loss_seg1 = seg_loss(pred1, labels)
            loss_seg2 = seg_loss(pred2, labels)
            loss = loss_seg2 + args.lambda_seg * loss_seg1  # 加不同尺度的约束
            ...
            # writer.add_image('target_image1',pred_target1,sub_i)
            # writer.add_image('target_image2',images,sub_i)
        ...
        if i_iter % args.save_pred_every == 0:
            ...
            mIoU = evaluate(model,pred_dir='/data12T/ydaugust/code/domain_adaptation/MetaCorrection-main/result/resnet101_gta_adv_25w_frombegin')
            writer.add_scalar('mIoU', mIoU, i_iter)
            print('Finish Evaluation: '+time.asctime(time.localtime(time.time())))
            if mIoU > best_mIoU:
                best_mIoU = mIoU
                torch.save(model.state_dict(), osp.join(args.snapshot_dir, 'resnet101_GTA5_best.pth'))
                torch.save(model_D1.state_dict(), osp.join(args.snapshot_dir, 'resnet101_GTA5_best_D1.pth'))
                torch.save(model_D2.state_dict(), osp.join(args.snapshot_dir, 'resnet101_GTA5_best_D2.pth'))
