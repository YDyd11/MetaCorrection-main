...
def fast_hist(a, b, n):
    k = (a >= 0) & (a < n)
    return np.bincount(n * a[k].astype(int) + b[k], minlength=n ** 2).reshape(n, n) # 这个的目的是什么？
...
def compute_mIoU(gt_dir, pred_dir,
    ...
    for ind in range(len(gt_imgs)):
        ...
        label = label_mapping(label, mapping) # 转换label ，映射label
...
def evaluate(seg_model, pred_dir='/data12T/ydaugust/code/domain_adaptation/MetaCorrection-main/result/cityscapes_resnet101_15+10'):
    ...
    for index, batch in enumerate(testloader):
        ...
        output = np.asarray(np.argmax(output, axis=2), dtype=np.uint8) # 返回一列中 axis=2中，最大值。 指出这个pixel属于哪一类。
...
if __name__ == '__main__':
    ...
    model.load_state_dict(torch.load('/data12T/ydaugust/code/domain_adaptation/MetaCorrection-main/snapshots/Past/resnet50_GTA5_best.pth'))
    ...
    evaluate(model, pred_dir='/data12T/ydaugust/code/domain_adaptation/MetaCorrection-main/log/ltir/result')
